...
You'll need these tools to get started with compiling and running the kernel/bootloader on qemu.
...
jq is usually installed on most Linux distros by default but you should check.

=== qemu
You'll need qemu to run `make run-qemu` build command which creates a bootable disk image and boot from it on qemu.
